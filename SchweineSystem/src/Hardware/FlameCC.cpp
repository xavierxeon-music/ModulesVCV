...
void FlameCC::process(const ProcessArgs& args)
{
   ...
   for (uint8_t index = 0; index < 16; index++)
   {
      ...
      const uint8_t midiValue = static_cast<uint8_t>(voltageToCcValue(voltage));

      midi::Message message;
      message.bytes[0] = Midi::Event::ControlChange & (Midi::Device::FlameCC - 1);
      message.bytes[1] = Midi::ControllerMessage::User01 + index;
      message.bytes[2] = midiValue;
